...
- `--save_best_by_cbi` — also save `best_model_by_cbi.pt` (best-by-val-CBI) alongside `best_model.pt` (best-by-val-AUC). At end-of-training, both checkpoints are eval'd on test and reported under `test_by_selection` in `ablation_summary.json`. Useful for focal runs where AUC and CBI peaks may diverge across epochs.
