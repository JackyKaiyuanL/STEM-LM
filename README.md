...
**Loss**
- `--loss_type {bce,focal}` (default `bce`) — sigmoid BCE or sigmoid focal loss (Lin et al. 2017). Focal trades AUC for CBI: in our eButterfly bench, `bce + class_weighting` got AUC@p=1=0.86, CBI=0.54; `focal γ=2 α=0.25 + class_weighting` got AUC@p=1=0.84, CBI=0.85. Caveat: focal **degrades probability-magnitude calibration** — ECE goes from ~0.013 (BCE) to ~0.075 (focal γ=2). For habitat-suitability mapping (uses ranking), focal wins. For occupancy modeling / population dynamics (uses absolute probabilities), prefer BCE or post-hoc-calibrate focal predictions.
- `--focal_alpha` (`0.25`), `--focal_gamma` (`2.0`) — RetinaNet defaults. γ is the AUC↔CBI lever; γ=1 is a milder trade than γ=2, γ=0 ≡ weighted BCE. α balances positive vs negative class; set <0 to disable α-balancing.

**Training**
- `--batch_size` (`32`), `--num_epochs` (`50`), `--learning_rate` (`1e-4`), `--weight_decay` (`0.01`).
- `--p` (default `0.15`) — per-row mask rate applied to both presences and absences. Accepts a float in `[0, 1]`, a `unif[:lo,hi]` string (sample `Uniform[lo, hi]` per row; bare `unif` = `unif:0.0,1.0`), or a `beta:alpha,beta` string (sample `Beta(alpha, beta)` per row).
...
- Both `best_model.pt` (best-by-val-AUC) and `best_model_by_cbi.pt` (best-by-val-CBI) are saved automatically. At end-of-training both are eval'd on test and reported under `test_by_selection.{auc,cbi}` in `ablation_summary.json`. When the val-AUC and val-CBI peaks fall on the same epoch the two checkpoints are identical; when they diverge (common for focal), use whichever matches your downstream criterion.
- `--absence_mask_eval` — run a second test block in addition to the standard uniform-mask block. Mask all absences plus `--absence_mask_p_list` fraction of presences (presence-only data scenario; default p list `0.25 0.5 0.75 1.0`). Reports under `absmask_*` keys in `ablation_summary.json`.
