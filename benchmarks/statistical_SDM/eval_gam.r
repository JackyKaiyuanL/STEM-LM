...
RESULTS_SUBDIR <- Sys.getenv("RESULTS_SUBDIR", unset = "gam")
COV_SETS_ENV   <- Sys.getenv("COV_SETS",       unset = "env,spatiotemporal,full")
results_dir <- file.path(PROJECT_DIR, DATASET, "results", RESULTS_SUBDIR)
metrics_dir <- file.path(results_dir, "metrics")
dir.create(metrics_dir, recursive = TRUE, showWarnings = FALSE)

COV_SETS <- strsplit(COV_SETS_ENV, ",")[[1]]
...
cat("\n--- GAM: full STEM-LM metric set (mean across species) ---\n")
