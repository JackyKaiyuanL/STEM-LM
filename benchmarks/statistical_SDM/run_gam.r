...
results_dir <- file.path(PROJECT_DIR, DATASET, "results", "gam")
...
if (has_time) {
  ...
  has_time_var <- length(unique(dat$doy)) > 1
} else {
  has_time_var <- FALSE
}
...
non_sp   <- c("time", "latitude", "longitude", "time_num", "doy", env_cols)
all_sp   <- setdiff(names(dat), non_sp)

cat(sprintf("Time encoding: %s\n", if (has_time_var) "DOY (cyclic smooth)" else "absent (s(doy) dropped)"))
...
writeLines(c(
  ...
  "model_spatiotemporal=bam(y ~ s(latitude,longitude,k=50) + s(doy,bs='cc',k=12), binomial)  # no s(time_num): observation-effort confound",
  "model_full=bam(y ~ env_* + s(latitude,longitude,k=50) + s(doy,bs='cc',k=12), binomial)  # no s(time_num): observation-effort confound"
), file.path(results_dir, "run_params.txt"))

.doy_term <- if (has_time_var) "+ s(doy, bs = 'cc', k = 12)" else ""

fit_one <- function(cov_set, y_tr, train_dat) {
  if (cov_set == "env") {
    fml <- as.formula(paste("y ~", paste(env_cols, collapse = " + ")))
    m <- tryCatch(
    ...
    fml <- as.formula(paste(
      "y ~ s(latitude, longitude, k = 50)", .doy_term
    ))
    m <- tryCatch(
      bam(fml, data = cbind(train_dat, y = y_tr), family = binomial(link = "logit"),
          discrete = TRUE, nthreads = 1),
      error = function(e) { warning(conditionMessage(e)); NULL }
    )
    if (is.null(m)) return(list(model = NULL, converged = FALSE, kind = "bam"))
    list(model = m, converged = isTRUE(m$converged), kind = "bam")
  } else {
    fml <- as.formula(paste(
      "y ~",
      paste(env_cols, collapse = " + "),
      "+ s(latitude, longitude, k = 50)", .doy_term
    ))
    m <- tryCatch(
      bam(fml, data = cbind(train_dat, y = y_tr), family = binomial(link = "logit"),
          discrete = TRUE, nthreads = 1),
